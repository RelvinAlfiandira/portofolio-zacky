...
export default function HeroSection() {
  return (
    <section className="relative min-h-[80vh] sm:min-h-[85vh] flex flex-col justify-between pt-6 sm:pt-8 pb-6 px-4 sm:px-8 bg-black rounded-3xl overflow-hidden border border-zinc-900">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-start relative z-10 mt-2 sm:mt-4">
        ...
        <motion.div 
          ...
          className="lg:col-span-4 space-y-2 sm:space-y-4"
        >
          <h2 className="text-lg sm:text-2xl font-light text-zinc-300 leading-snug drop-shadow-md">
            Backend Developer <br className="hidden sm:inline" />
            <span className="text-zinc-400 sm:inline"> based in Indonesia</span>
          </h2>
        </motion.div>

        <div className="hidden lg:block lg:col-span-3" />

        {/* Bio & Uniform CTA Buttons */}
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <p className="text-zinc-300 sm:text-zinc-400 text-sm sm:text-base leading-relaxed drop-shadow">
          ...
          <div className="pt-1 sm:pt-2 flex flex-wrap items-center gap-3">
            ...
            <a
              ...
              className="inline-flex items-center justify-center h-11 px-6 border border-zinc-700 hover:border-zinc-500 bg-black/40 backdrop-blur-sm text-zinc-200 hover:text-white font-medium rounded-full transition-colors text-xs sm:text-sm pointer-events-auto"
            >
              Let's Connect
            </a>
          </div>
      ...
      >
        <h1 className="text-2xl sm:text-4xl md:text-5xl lg:text-5xl xl:text-6xl font-extrabold tracking-tight text-white uppercase select-none leading-none drop-shadow-lg">
          ZACKY AL GHIFARI
        </h1>
      </motion.div>
    </section>
